...
def create_attachment_by_customer(
    db: Session, ticket_id: int, customer_id: int, file: UploadFile
) -> InteractionHistory:
    """
    Khách hàng ĐÃ ĐĂNG NHẬP gửi kèm 1 ảnh/video vào ticket của chính mình, dùng
    ở trang "Tài khoản của tôi" — bản sao logic của
    public_chat_service.submit_attachment() (dành cho chat ẩn danh), nhưng xác
    thực quyền sở hữu ticket qua customer_id (JWT) thay vì access_token.
    Cũng tự động mở lại ticket 'closed' -> 'new', giống create_message_by_customer().
    """
    ticket = get_ticket_or_404(db, ticket_id)
    if ticket.customer_id != customer_id:
        raise HTTPException(
